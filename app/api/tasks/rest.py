...
task_router_rest = APIRouter()

@task_router_rest.get("/", response_model=Page[schemas.TaskOut])
async def list_tasks(current_user: User = Depends(get_current_user)):
    """List all tasks for user."""
    return await paginate(Task.filter(user=current_user))


@task_router_rest.get(
    "/{task_id}",
...
@task_router_rest.put("/{task_id}", response_model=schemas.TaskOut)
...
@task_router_rest.delete("/{task_id}", status_code=HTTP_204_NO_CONTENT)
